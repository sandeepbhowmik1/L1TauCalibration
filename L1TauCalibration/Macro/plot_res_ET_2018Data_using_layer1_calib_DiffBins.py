...
for i in range (0, len(PtBinsName)):
    ...
    for j in range (0, len(EtaBinsName)):
        ...
        hEt_raw_Data = TH1F ("hEt_raw_Data" + PtBinsName[i] + "_" + EtaBinsName[j], "hEt_raw_Data" + PtBinsName[i] + "_" + EtaBinsName[j], 60, 0, 3)
        hEt_calib_Data = TH1F ( "hEt_calib_Data" + PtBinsName[i] + "_" + EtaBinsName[j], "hEt_calib_Data" + PtBinsName[i] + "_" + EtaBinsName[j], 60, 0, 3)
        ...
        for iEv in range (0, nEv):
            ...
            if tData.OfflineTau_pt < PtBins[i] or tData.OfflineTau_pt >= PtBins[i+1] :
                continue

            if abs(tData.OfflineTau_eta) < EtaBins[j] or abs(tData.OfflineTau_eta) >= EtaBins[j+1] :
                continue

            hEt_raw_Data.Fill(tData.L1Tau_RawIEt/2 / tData.OfflineTau_pt)
            hEt_calib_Data.Fill(tData.L1Tau_pt / tData.OfflineTau_pt)
            #hEt_calib_MC_18.Fill(tData.L1Tau_pt / tData.OfflineTau_pt)
        ...
        leg.AddEntry(hEt_raw_Data, "Raw L1", "l")
        ...
        leg.AddEntry(hEt_calib_Data, "Calibrated L1 ", "l")
        ...
        hEt_calib_Data.SetLineColor(kRed)

        hEt_calib_Data.SetTitle(";E_{T}^{#tau, L1} / p_{T}^{#tau, offline}; a.u.")

        #hEt_calib_MC_18.DrawNormalized("")
        hEt_calib_Data.DrawNormalized("")
        ...
        c1.Print("/home/sbhowmik/L1TauTrigger_NTuple/Performance_2018data_181009/plots/resolution_ET_Data_" + PtBinsName[i] + "_" + EtaBinsName[j] + "_181119.pdf", "pdf")
        c1.Print("/home/sbhowmik/L1TauTrigger_NTuple/Performance_2018data_181009/plots/resolution_ET_Data_" + PtBinsName[i] + "_" + EtaBinsName[j] + "_181119.png", "png")
